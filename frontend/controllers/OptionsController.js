...
export async function createOption(start, end, votes) {
  try {
    ...
    const response = (
      await axios.post(url + "/options/" + token, {
        start: start,
        end: end,
        votes: votes,
      })
...
export async function getOption(optionId) {
  try {
    const response = (
      await axios.get(url + "/option/" + optionId + "&" + token)
    ).data;
    if (response.status === 200) {
      return response.data;
    } else {
...
export async function addVote(optionId, userId, groupId) {
  try {
    const response = (
      await axios.post(
        url + "/option/addVote/" + optionId + "&" + userId + "&" + token
      )
    ).data;
    const response = (
      await axios.post(
        url + "/option/addVote/" + optionId + "&" + userId + "&" + groupId
      )
    ).data;
    if (response.status === 200) {
      return response.data;
